...
class THEMIS_NASH(DefaultParty):

    def __init__(self):
        super().__init__()
        self._best_bids_so_far = []
        self._prop_outcome = 1
        self._p = 0.9                                   # ODDS OF REPEATING A BID IN THE EXPLORATORY PHASE
        self._kindness=0
    ...
    def _eval_bid(self, bid: Bid, profile)->Decimal:
        ...
        return self._c1*profile.getUtility(bid)-self._c2*self._fairness_function(us,op,self._nash_best)
    ...
    def _findBid(self) -> Bid:
        ...
        if self._state >= 1 and self._state<4:
            ...
            if len(self._prefer_bits)==0:
                ...
                all_bids = AllBidsList(domain)
                n = all_bids.size()
                self._nash_best = Decimal(0)
                for i in range(n):
                    
                    if(Decimal(self._op_profile(all_bids.get(i))*profile.getUtility(all_bids.get(i)))>self._nash_best):
                        self._nash_best = Decimal(self._op_profile(all_bids.get(i))*profile.getUtility(all_bids.get(i)))
            ...
            self._fairness_function(profile.getUtility(out),Decimal(self._op_profile(out)),self._nash_best),
            self._dynamic_fairness(profile.getUtility(out),Decimal(self._op_profile(out)),self._nash_best),
    ...
    def _fairness_function(self, us: Decimal, opponent: Decimal, target: Decimal) -> Decimal:
        ...
        # else:
        #     out = degrees(abs(fraction-atan(prop)))/90
        out = 1 - (us*opponent)/float(target)
